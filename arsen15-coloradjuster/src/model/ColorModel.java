 ...
 * 
 * <p>Arsen Shintemirov edited the setRed(), setGreen(), and setBlue() methods, and
 * created a helper method changingColor().
 * 
...
public class ColorModel implements PropertyChangeEnabledMutableColor {
    ...
    public ColorModel(final int theRed, 
        ...
        if (theRed < MIN_VALUE || theRed > MAX_VALUE) {
            throw new IllegalArgumentException("The value for color red must be [0-255]. ");
        }
        if (theGreen < MIN_VALUE || theGreen > MAX_VALUE) {
            throw new IllegalArgumentException("The value for color green must be [0-255]. ");
        }
        if (theBlue < MIN_VALUE || theBlue > MAX_VALUE) {
            throw new IllegalArgumentException("The value for color blue must be [0-255]. ");
        }
        myRed = theRed;
        myGreen = theGreen;
        myBlue = theBlue;
    ...
    public void setBlue(final int theBlue) {
        ...
        //Notifying the change in blue value.
    ...
    /**
     * Helper method that updates the entire Color object
     * with the new values.
     */
    private void changingColor() {
        
      //Changing the entire color.
        //old color 
        final Color oldColor = this.getColor();
        
        //new color
        final Color color = new Color(myRed, myGreen, myBlue);
        
        myPcs.firePropertyChange(PROPERTY_COLOR, oldColor, color);
    }
